...
export class PaymentsService {
  ...
  constructor(
    ...
    const apiUrl = this.configService.get(ENV.LEMONSQUEEZY_API_URL);
    const apiKey = this.configService.get(ENV.LEMONSQUEEZY_API_KEY);
  ...
  async createCheckout(productId: string, userId: string) {
    const storeId = this.configService.get(ENV.LEMONSQUEEZY_STORE_ID);
  ...
  async handleWebhook(signature: string, rawBody: string, eventType: string, body: any) {
    const secret = this.configService.get(ENV.LEMONSQUEEZY_WEBHOOK_SIGNATURE);
